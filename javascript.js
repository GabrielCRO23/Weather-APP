...
async function getWeatherC() {
  UNITS = "units=metric";
  let CITY_NAME =
    document.getElementById("weather-search").value || "dubrovnik";
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${CITY_NAME}&appid=570f50d767691ae0612a59f3d17eac1a&${UNITS}`,
    { mode: "cors" }
  );
...
async function getWeatherF() {
  ...
  details.textContent = data.weather[0].description.toUpperCase();
  FEELS_LIKE.textContent =
    "FEELS LIKE:" + " " + Math.floor(data.main.feels_like) + "°F";
}
...
// BELOW IS THE COUNTRY FLAG CONSTRUCTOR
...
class CountryFlag {
  ...
  randomize() {
    ...
    const randomIndex = Math.floor(
      Math.random() * CountryFlag.countryList.length
    );
  ...
  static getCountryByAlpha2(alpha2) {
    const country = CountryFlag.getCountryByMapName(
      CountryFlag.IDX_ALPHA2,
      alpha2
    );
  ...
  trySelectCountry(country) {
    if (country) {
      this.element.setAttribute(
        "class",
        `flag flag-${country[CountryFlag.IDX_ALPHA2]}`
      );
